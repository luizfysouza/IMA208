...
def me_sad(cur, ref, brow, bcol, search, lamb=0):
    ...
    if lamb == 0.:
        for i in range(0, np.shape(cur)[0], brow) : # for each block in the current image, find the best corresponding block in the reference image
            for j in range(0, np.shape(cur)[1], bcol) :
                ...
                for dcol in range (-search, search): # dcol = candidate displacement vector over the columns
                    for drow in range (-search, search):# rcol = candidate displacement vector over the rows
                        ref_Ver = search  + drow + i 
                        ref_Hor = search  + dcol + j 
                        
                        ref_block = ref_extended[ref_Ver:ref_Ver+brow, ref_Hor:ref_Hor+bcol] 
                        cost = np.sum(np.abs(ref_block - B))
                        ...
                        if cost<costMin: # Save the results if they are better than the previous ones
                            ...
                            djmin = dcol
                            dimin = drow
                mvf[i:i+brow,j:j+bcol,0] = dimin # Once the loop is over, save the best row displacement field
                mvf[i:i+brow,j:j+bcol,1] = djmin # Once the loop is over, save the best column displacement field
            ...
            for j in range(0,np.shape(cur)[1], bcol):
                ...
                for dcol in range(-search, search, 1): # dcol = candidate displacement vector over the columns
                    for drow in range(-search, search, 1):# rcol = candidate displacement vector over the rows
                        ref_Ver = search + i + drow 
                        ref_Hor = search + j + dcol 
                    
                        ref_block = ref_extended[ref_Ver:ref_Ver+brow, ref_Hor:ref_Hor+bcol] 
                        cost = np.sum(np.abs(ref_block - B)) + lamb*((drow-pV[0])**2+(dcol-pV[1])**2)
